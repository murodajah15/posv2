...
class DashboardController extends Controller
{
  ...
  public function index(Request $request) //: View
  {
    ...
    foreach ($databelitahunan as $dj) {
      ...
      $beli = Belih::where('proses', 'Y')->whereMonth('tglbeli', '<=', $dj->monthn)
        ->whereYear('tglbeli', $tahun)
        ->sum('total');
      $pelunasan = Kasir_keluar::where('proses', 'Y')->whereMonth('tglkwitansi', '<=', $dj->monthn)
        ->whereYear('tglkwitansi', $tahun)
        ->sum('subtotal');
      $totalhutang[] = ($beli - $pelunasan) / 1000000;
    }
  ...
  public function grafik(Request $request)
  {
    if ($request->Ajax()) {
      ...
      foreach ($databelitahunan as $dj) {
        $categoriestahunan[] = $dj->monthname;
        $totalbelitahunan[] = (Belih::where('proses', 'Y')->whereMonth('tglbeli', $dj->monthn)->whereYear('tglbeli', $tahun)->sum('total')) / 1000000;
        $hutang = Belih::where('proses', 'Y')->whereMonth('tglbeli', '<=', $dj->monthn)
          ->whereYear('tglbeli', $tahun)
          ->sum('kurangbayar');
        $hutang1 = Belih::where('proses', 'Y')->whereYear('tglbeli', $tahun - 1)
          ->sum('kurangbayar');
        $totalhutang[] = ($hutang + $hutang1) / 1000000;
      }
      $databeli = Belih::selectRaw('year(tglbeli) year, monthname(tglbeli) monthname,month(tglbeli) monthn, count(*) data')
        ->where('proses', 'Y') //->whereMonth('tglbeli', '>', 8)
        ->whereYear('tglbeli', $tahun)
      ...
      //piutang
      foreach ($datajualtahunan as $dj) {
        $piutang = Jualh::where('proses', 'Y')->whereMonth('tgljual', '<=', $dj->monthn)
          ->whereYear('tgljual', $tahun)
          ->sum('kurangbayar');
        $piutang1 = Jualh::where('proses', 'Y')->whereYear('tgljual', $tahun - 1)
          ->sum('kurangbayar');
        $totalpiutang[] = ($piutang + $piutang1) / 1000000;
      }
      // //piutang
      // foreach ($datajualtahunan as $dj) {
      //   $jual = Jualh::where('proses', 'Y')->whereMonth('tgljual', '<=', $dj->monthn)
