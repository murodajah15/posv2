    ...
    <div class="modal-content">
        <div class="modal-header">
            <h5 class="modal-title" id="staticBackdropLabel">
                {{ $vdata['title'] . ' No : ' . $approv_batas_piutang->noapprov }}
            </h5>
            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                <span aria-hidden="true">&times;</span>
            </button>
        </div>
        <form action="{{ $action }}" method="post" class="formbatalproses">
            @csrf
            ...
            <div class="modal-body">
                <div class="row">
                    <div class="col-lg-12 margin-tb">
                        <textarea rows="4" class="form-control" id="catatan" name='catatan' required></textarea>
                    </div>
                </div>
...
<script>
    $(document).ready(function() {
        var myModal = document.getElementById('modalbatalproses')
        var myInput = document.getElementById('catatan')
        $(myModal).on('shown.bs.modal', function() {
            $(this).find(myInput).focus();
        });

        $('.formbatalproses').submit(function(e) {
            const form = $(this)
            e.preventDefault();
            $.ajax({
                type: "POST",
