...
if ($llogo == 'Y' and $logo != '') {
    ...
    if (env('APP_URL') == 'http://localhost') {
        ...
        if (file_exists($filename)) {
            $html .= '<td><img src=' . $filename . ' width="20"></td>';
        }
    } else {
        $filename = Session::get('logo', 'default');
        if (File::exists('../../pos/storage/app/public/uploads/' . $filename)) {
            $html .= '<td><img src=' . '../../pos/storage/app/public/uploads/' . $filename . ' width="20"></td>';
        }
    }
}
...
$html .= '<td style="font-size:7px">' . session('nm_perusahaan') . '<br><span style="font-size:4px;">' . session('alamat_perusahaan') . '<br>' . session('telp_perusahaan') . '</font></td></tr></table>';
