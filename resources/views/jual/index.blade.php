    ...
    <div class="content-wrapper">
        ...
        <div class="content-header">
            <div class="container-fluid">
                ...
                {{-- <input type='hidden' name='closing_hpp' id ='closing_hpp' value="{{ $saplikasi->closing_hpp }}"> --}}
                <div class="card mt-2">
                    <div class="card-body">
                        <div class="table-responsive">
                            <table id="tbl-jual" class="table table-bordered table-hover table-sm tbl-jual">
                                {{-- <table id="so" class="table table-bordered table-striped so"> --}}
                                <thead>
                                    <tr>
                                        <th width=30 style="text-align:center;">#</th>
                                        <th width="100">No. Jual</th>
                                        <th width="80">Tanggal</th>
                                        <th width="450">Customer</th>
        ...
        <div class="modal fade" id="modalcaritbmultiprc" data-backdrop="static" data-keyboard="false" tabindex="-1"
            aria-labelledby="staticBackdropLabel" aria-hidden="true">
        </div>
        <div class="modal fade" id="modalbatalproses" data-backdrop="static" data-keyboard="false" tabindex="-1"
            aria-labelledby="staticBackdropLabel" aria-hidden="true">
        </div>
        <div class="modal fade" id="modalcariso" data-backdrop="static" data-keyboard="false" tabindex="-1"
            aria-labelledby="staticBackdropLabel" aria-hidden="true">
        </div>
        ...
        <script>
                    ...
                    var table = $('.tbl-jual').DataTable({
                        ...
                        columns: [{
                            ...
                            },
                            {
                                ...
                                name: 'nojual',
                                render: function(data, type, row, meta) {
                                    return `<a href="#" onclick="detail(${row.id})">${row.nojual}</a>`;
                                }
                            },
                            {
                                data: 'tgljual',
                            ...
                            },
                            {
                                data: 'sudahbayar',
                                render: function(data, type, row, meta) {
                                    return meta.settings.fnFormatNumber(row.sudahbayar);
                                }
                            },
                            // {
                            //     data: 'proses',
                            ...
                            //     </div>`
                                    ...
                                    if (row.batal == 'Y') {
                                        ...
                                        <ul class="dropdown-menu" aria-labelledby="btnGroupDrop1">
                                            ...
                                            <li><a class="dropdown-item <?= $edit == 1 ? '' : 'disabled' ?>" onclick="input_juald(${row.id})" href="#" readonly><i class='fa fa-book'"></i> Detail</a></li>
                        ...
                        if (willDelete) {
                            $.ajax({
                                ...
                                success: function(response) {
                                    if (response.sukses == false) {
                                        toastr.warning(
                                            'Data gagal di proses (total masih 0 / Stock kurang)')
                                    } else {
                                        reload_table();
                    ...
                    success: function(response) {
                        if (response.data) {
                            ...
                            if (JSON.parse(response) == '2') {
                                toastr.error(
                                    'Sudah ada pembayaran, data tidak bisa di Unproses!'
                                );
                            } else {
                                toastr.error(
                                    'Sudah closing bulanan, data tidak bisa di Unproses!'
                                );
                            }
                        }
                    },
            ...
            function input_juald(id) {
                $.ajax({
                    type: "get",
                    url: `{{ url('jualinputjuald') }}`,
                    dataType: "json",
                    data: {
                        id: id,
                        methode: "get",
            ...
            function cetakfp(id) {
                $url = "{{ url('jualcetakfp') }}?id=" + id
                window.open($url, '_blank')
            }

            function cetaksj(id) {
                $url = "{{ url('jualcetaksj') }}?id=" + id
                window.open($url, '_blank')
            }
