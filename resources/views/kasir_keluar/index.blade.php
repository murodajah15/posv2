...
@section('content')
    ...
    <div class="content-wrapper">
        ...
        <div class="content-header">
            <div class="container-fluid">
                ...
                <div class="card mt-2">
                    <div class="card-body">
                        <table id="tbl-kasir_keluar" class="table table-bordered table-hover table-sm tbl-kasir_keluar">
                            {{-- <table id="tbl-kasir_keluar" class="table table-bordered table-striped tbl-kasir_keluar"> --}}
                            <thead>
                                <tr>
    ...
    <div class="modal fade" id="modalcarimohklruang" data-backdrop="static" data-keyboard="false" tabindex="-1"
        aria-labelledby="staticBackdropLabel" aria-hidden="true">
    </div>
    <div class="modal fade" id="modalcaribeli" data-backdrop="static" data-keyboard="false" tabindex="-1"
        aria-labelledby="staticBackdropLabel" aria-hidden="true">
    </div>
    ...
    <script>
                ...
                var table = $('.tbl-kasir_keluar').DataTable({
                    ...
                    columns: [{
                        ...
                        },
                        {
                            ...
                            // data: null,
                            render: function(data, type, row, meta) {
                                return `<a href="#" onclick="detail(${row.id})">${row.nokwitansi}</a>`;
                            }
                        },
                        {
                            data: 'tglkwitansi',
                                ...
                                if (row.batal == 'Y') {
                                    ...
                                    if (row.proses == 'Y') {
                                        ...
                                        <ul class="dropdown-menu" aria-labelledby="btnGroupDrop1">
                                            ...
                                            <li><a class="dropdown-item <?= $edit == 1 ? '' : 'disabled' ?>" onclick="input_kasir_keluard(${row.id})" href="#" readonly><i class='fa fa-book'"></i> Detail</a></li>
        ...
        function reload_table1() {
            $(document).ready(function() {
                $('#tbl-kasir_keluar-data').DataTable({
                    // "destroy": true,
                    "aLengthMenu": [
                        [5, 50, 100, -1],
                        [5, 50, 100, "All"]
            ...
            $.ajax({
                url: "<?= 'tabel_kasir_keluar' ?>",
                beforeSend: function(f) {
                    $('.btnreload').attr('disable', 'disabled')
                    $('.btnreload').html('<i class="fa fa-spin fa-spinner"></i>')
                    // alert('1');
                    $('#tabel_kasir_keluar').html('<center>Loading Table ...</center>');
                },
                success: function(data) {
                    // alert(data);
                    $('#tabel_kasir_keluar').html(data);
        ...
        function input_kasir_keluard(id) {
            $.ajax({
                type: "get",
                url: `{{ url('kasir_keluarinputkasir_keluard') }}`,
                dataType: "json",
                data: {
                    id: id,
                    methode: "get",
        ...
        function unproses(id) {
            ...
            $.ajax({
                type: "get",
                url: `{{ url('kasir_keluarbatalproses') }}`,
                dataType: "json",
                data: {
                    id: id,
                    methode: "get",
        ...
        function cetak(id) {
            $url = "{{ url('kasir_keluarcetak') }}?id=" + id
