...
@section('content')
    ...
    <div class="content-wrapper">
        ...
        <div class="content-header">
            <div class="container-fluid">
                ...
                <div class="card mt-2">
                    <div class="card-body">
                        <table id="tbl-kasir_tagihan" class="table table-bordered table-hover table-sm tbl-kasir_tagihan">
                            {{-- <table id="tbl-kasir_tagihan" class="table table-bordered table-striped tbl-kasir_tagihan"> --}}
                            <thead>
                                <tr>
    ...
    <div class="modal fade" id="modalcarijual" data-backdrop="static" data-keyboard="false" tabindex="-1"
        aria-labelledby="staticBackdropLabel" aria-hidden="true">
    </div>
    ...
    <script>
                                ...
                                if (row.batal == 'Y') {
                                    ...
                                    if (row.proses == 'Y') {
                                        ...
                                        <ul class="dropdown-menu" aria-labelledby="btnGroupDrop1">
                                            ...
                                            <li><a class="dropdown-item <?= $edit == 1 ? '' : 'disabled' ?>" onclick="input_kasir_tagihand(${row.id})" href="#" readonly><i class='fa fa-book'"></i> Detail</a></li>
        ...
        function reload_table1() {
            $(document).ready(function() {
                $('#tbl-kasir_tagihan-data').DataTable({
                    // "destroy": true,
                    "aLengthMenu": [
                        [5, 50, 100, -1],
                        [5, 50, 100, "All"]
            ...
            $.ajax({
                url: "<?= 'tabel_kasir_tagihan' ?>",
                beforeSend: function(f) {
                    $('.btnreload').attr('disable', 'disabled')
                    $('.btnreload').html('<i class="fa fa-spin fa-spinner"></i>')
                    // alert('1');
                    $('#tabel_kasir_tagihan').html('<center>Loading Table ...</center>');
                },
                success: function(data) {
                    // alert(data);
                    $('#tabel_kasir_tagihan').html(data);
        ...
        $('.tomboltambah').click(function(e) {
            e.preventDefault();
            $.ajax({
                url: `{{ route('kasir_tagihan.create') }}`,
                dataType: "json",
        ...
        function input_kasir_tagihand(id) {
            $.ajax({
                type: "get",
                url: `{{ url('kasir_tagihaninputkasir_tagihand') }}`,
                dataType: "json",
                data: {
                    id: id,
                    methode: "get",
        ...
        function proses(id) {
            swal({
                ...
                .then((willDelete) => {
                    if (willDelete) {
                        $.ajax({
                            url: `{{ url('kasir_tagihanproses') }}/${id}`,
                            type: "POST",
                            data: {
                                id: id,
                                _token: '{{ csrf_token() }}',
        ...
        function unproses(id) {
            ...
            $.ajax({
                type: "get",
                url: `{{ url('kasir_tagihanbatalproses') }}`,
                dataType: "json",
                data: {
                    id: id,
                    methode: "get",
        ...
        function cetak(id) {
            $url = "{{ url('kasir_tagihancetak') }}?id=" + id
