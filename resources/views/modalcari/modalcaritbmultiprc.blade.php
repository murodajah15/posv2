            ...
            <div class="modal-body">
                <div class="row">
                    <div class="col-lg-12 margin-tb">
                        <table id="tblcarimultiprc" class="table table-bordered table-hover table-sm"
                            style="width:100%">
                            <thead>
                                <tr>
                                    <th width="5">No</th>
                            ...
                            <tbody id="isi_data" class="isi_data">
                                ...
                                @foreach ($tbmultiprc as $row)
                                    <?php $no++;
                                    $satuan = $row->kdsatuan . '-' . $row->nmsatuan;
                                    ?>
                                    <tr id="tr" style="cursor: pointer; ">
                                        <td width="5" style="text-align:center;"><?= $no ?></td>
                                        <td width="70" id="td" data-bs-dismiss="modal"
                                            onclick="post_data_tbmultiprc();">
                                            {{ $row->kode }}</td>
                                        <td width="350" id="td" data-bs-dismiss="modal"
                                            onclick="post_data_tbmultiprc();">
                                            {{ $row->nama }}</td>
                                        <td width="90" id="td" data-bs-dismiss="modal"
                                            onclick="post_data_tbmultiprc();">
                                            {{ $satuan }}</td>
                                        <td style="text-align:right;" id="td" data-bs-dismiss="modal"
                                            onclick="post_data_tbmultiprc();">
                                            {{ $row->harga }}</td>
                                        <td id="td" hidden data-bs-dismiss="modal"
                                            onclick="post_data_tbmultiprc();">
                                            {{ $row->kdsatuan }}</td>
                                    </tr>
                                @endforeach
...
<script>
    ...
    function post_data_tbmultiprc() {
        var table = document.getElementById("tblcarimultiprc");
        ...
        tbody.onclick = function(e) {
            ...
            $('#kdsatuan').val(((dt_split[5]).replace("--separator--", "")).trim());
            $('#harga').val(((dt_split[4]).replace("--separator--", "")).trim());
            $('#modalcaritbmultiprc').modal('hide');
