...
if ($rincian == 'Y') {
    if ($groupingcustomer == 'Y') {
        ...
        while ($rowklp = mysqli_fetch_assoc($qryklp)) {
            ...
            <td colspan=13 height="10px" align="left"><b>Kelompok Customer : ' .
                ...
                '</b></td>';
            // }
            $qrygroupcust = mysqli_query($connect, "select jualh.*,tbcustomer.kdklpcust from jualh left outer join tbcustomer on jualh.kdcustomer=tbcustomer.kode where jualh.proses='Y' and tbcustomer.kdklpcust='$kdklpcust' group by jualh.kdcustomer order by jualh.kdcustomer");
            $ng = 0;
            $gtsubtotal = 0;
            $gtdiscount = 0;
            $gtbayar = 0;
                ...
                while ($row = mysqli_fetch_assoc($queryh)) {
                    ...
                    if ($semuabarang == 'Y') {
                        ...
                        $queryd = mysqli_query(
                            ...
                            where jualh.proses='Y' and jualh.kdcustomer='$kdcustomer' and juald.nojual='$nojual'",
                        );
                    } else {
                        $queryd = mysqli_query(
                            $connect,
                            "select juald.*,jualh.sudahbayar,jualh.kurangbayar from juald inner join jualh on jualh.nojual=juald.nojual
                            where jualh.proses='Y' and jualh.kdcustomer='$kdcustomer' and juald.nojual='$nojual' juald.kdbarang='$kdbarang'",
                        );
                    }
    ...
    while ($rowklp = mysqli_fetch_assoc($qryklp)) {
        ...
        $qh .= $groupingcustomer == 'Y' ? ' order by kdcustomer,tgljual' : 'group by nojual order by tgljual,nojual';
        $kdcustomer = '';
        $nocust = 0;
        $gtsubtotal = 0;
        $gtdiscount = 0;
