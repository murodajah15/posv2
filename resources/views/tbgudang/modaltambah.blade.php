...
<div class="modal-dialog" style="max-width: 50%;">
    <div class="modal-content">
        ...
        <form action="{{ $action }}" method="post" class="formtbgudang">
            @csrf
            @if ($tbgudang->id)
                @method('put')
            @endif
            <div class="modal-body">
                ...
                <div class="col-md-6 mb-2">
                    ...
                    <input type="text" class="form-control" name="kode" id="kode"
                        value="<?= $tbgudang->kode ?>" {{ str_contains($vdata['title'], 'Detail') ? 'readonly' : '' }}
                        {{ str_contains($vdata['title'], 'Detail') ? '' : 'autofocus' }}>
                </div>
                <div class="col-md-12 mb-2">
                    <label for="nama" class="form-label mb-1">Nama</label>
                    <input type="text" class="form-control" name="nama" id="nama"
                        value="<?= $tbgudang->nama ?>" {{ str_contains($vdata['title'], 'Detail') ? 'readonly' : '' }}>
                </div>
                <?php
...
<script>
    ...
    $(document).ready(function() {
        $('.formtbgudang').submit(function(e) {
            const form = $(this)
            e.preventDefault();
            $.ajax({
                type: "POST",
