...
<div class="modal-dialog" style="max-width: 50%;">
    <div class="modal-content">
        ...
        @endif --}}
        <form action="{{ $action }}" method="post" class="formtbjnkeluar">
            @csrf
            @if ($tbjnkeluar->id)
                @method('put')
            @endif
            <div class="modal-body">
                ...
                <input type="hidden" class="form-control-sm" name="kodelama" id="kodelama"
                    value="{{ $tbjnkeluar->kode }}">
                <div class="col-md-6 mb-2">
                    <label for="kode" class="form-label mb-1">Kode</label>
                    <input type="text" class="form-control" name="kode" id="kode"
                        value="{{ $tbjnkeluar->kode }}" {{ str_contains($vdata['title'], 'Detail') ? 'readonly' : '' }}
                ...
                <div class="col-md-12 mb-2">
                    ...
                    <input type="text" class="form-control" name="nama" id="nama"
                        value="{{ $tbjnkeluar->nama }}" {{ str_contains($vdata['title'], 'Detail') ? 'readonly' : '' }}>
                    @error('nama')
                        <span class="text-danger">{{ $message }}</span>
                    @enderror
                ...
                <div class="col-md-12 mb-2">
                    ...
                    <input type="text" class="form-control" name="user" id="user"
                        value="{{ $tbjnkeluar->user }}" readonly>
                </div> <?php } ?>
            </div>
...
<script>
    ...
    $(document).ready(function() {
        $('.formtbjnkeluar').submit(function(e) {
            const form = $(this)
            e.preventDefault();
            $.ajax({
                type: "POST",
